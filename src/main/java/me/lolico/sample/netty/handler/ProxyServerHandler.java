            ...
            new Bootstrap()
                    ...
                    .addListener((ChannelFutureListener) future -> {
                        if (future.isSuccess()) {
                            ...
                            // 所以无论是连接客户端的 localChannel 还是连接远端主机的 remoteChannel 都只需要一个 ForwardingHandler 就行了。
                            // 代理服务器在中间做转发。
                            // 客户端   --->  localChannel  --->  tunnel ---> remoteChannel ---> 远端主机
                            // 远端主机 --->  remoteChannel  --->  tunnel ---> localChannel ---> 客户端
